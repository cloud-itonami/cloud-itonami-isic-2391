(ns refractorymfg.advisor
  "RefractoryAdvisor -- the *contained intelligence node* for the
  refractory products (fire brick, kiln lining, furnace lining
  materials) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/weight/
  thermal-shock-cycles/cold-crushing-strength-mpa), drafts a
  pressing-line/kiln-line-equipment maintenance scheduling proposal
  against a piece of equipment, drafts a safety-concern flag, and
  drafts an outbound refractory-product shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real pressing-line/
  kiln-line actuation or freight dispatch. Every output is censored
  downstream by `refractorymfg.governor` before anything touches the
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `refractorymfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct pressing-
  ...
  `refractorymfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [kotoba.lang.text :as str]
            [refractorymfg.registry :as registry]
...
(defn- log-production-batch
  ...
  product-type, weight, thermal-shock-cycles, cold-crushing-strength
...
(defn- schedule-maintenance
  "Draft a pressing-line/kiln-line-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `refractorymfg.governor` NEVER trusts this report --
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-kiln-pressing-line?=" (boolean (:actuate-kiln-pressing-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-kiln-pressing-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a kiln-safety/thermal-hazard (kiln-firing burn/heat exposure),
  refractory-dust-hazard (crystalline silica/alumina/magnesia dust
  exposure at batching/pressing), or pressing-line-equipment safety
  ...
  technicality). See `refractorymfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `refractorymfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound refractory-product shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed weight -- it does NOT invent one, and
  `refractorymfg.governor` NEVER trusts it: it independently
...
(def ^:private system-prompt
  (str "あなたは耐火物(れんが・炉裏張り材)工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "プレス成形機・窯業焼成ライン(トンネル窯・シャトル窯等)等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `refractorymfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :refractorymfg-advisor-proposal
   :op         (:op request)
