(ns refractorymfg.governor
  "Refractory Plant Operations Governor -- the independent compliance
  layer that earns the RefractoryAdvisor the right to commit. The
  ...
  pressing-line press or kiln line, whether a shipment proposal's own
  ...
  becomes direct pressing-line/kiln-line-equipment control, so this
  MUST be a separate system able to *reject* a proposal and fall back
  to HOLD.

  `:itonami.blueprint/governor` is `:refractory-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       or `:pressing-line/run`) is the
                                       'direct pressing-line/kiln-line-
                                       equipment control' scope
                                       violation this actor must NEVER
                                       perform -- HARD, PERMANENT,
                                       unconditional.
    4. Kiln/pressing-line-actuate
       blocked                     -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:actuate-kiln-
                                       pressing-line? true`? Directly
                                       actuating the pressing-line press
                                       or kiln line is this actor's
    ...
    9. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       through.
   10. Invalid thermal-shock
       reading                      -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:thermal-shock-cycles` that is
   ...
   11. Invalid cold-crushing-
       strength reading             -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:cold-crushing-strength-mpa`
                                       that is not a physically
                                       plausible reading
                                       (`refractorymfg.registry/cold-
                                       crushing-strength-valid?`), the
   ...
   12. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [refractorymfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct pressing-line/
...
(defn- kiln-line-control-blocked-violations
  ...
  Anything else (direct pressing-line/kiln-line control, a fabricated
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :kiln-line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はプレス成形機・窯業焼成ライン等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- kiln-line-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-kiln-pressing-line? true` is
  attempting to directly actuate the pressing-line press or kiln line
  -- this actor may only ever propose/schedule a DRAFT maintenance
  window, never actuate the pressing-line press/kiln line directly. No
  override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-kiln-pressing-line? (:value proposal))))
    [{:rule :kiln-line-actuate-blocked
      :detail "プレス成形機・窯業焼成ライン(トンネル窯・シャトル窯等)の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-thermal-shock-violations
  "For `:log-production-batch`, if the patch declares a
  `:thermal-shock-cycles` that is not a physically plausible reading,
  reject rather than let fabricated/gauge-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [ts (:thermal-shock-cycles (:value proposal))]
      (when (and (some? ts) (not (registry/thermal-shock-cycles-valid? ts)))
        [{:rule :invalid-thermal-shock-cycles
          :detail (str ts " サイクルは物理的に妥当な耐熱衝撃性の範囲外")}]))))

(defn- invalid-cold-crushing-strength-violations
  "For `:log-production-batch`, if the patch declares a
  `:cold-crushing-strength-mpa` that is not a physically plausible
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [ccs (:cold-crushing-strength-mpa (:value proposal))]
      (when (and (some? ccs) (not (registry/cold-crushing-strength-valid? ccs)))
        [{:rule :invalid-cold-crushing-strength
          :detail (str ccs "MPa は物理的に妥当な常温圧縮強度の範囲外")}]))))

(defn check
  "Censors a RefractoryAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-thermal-shock-violations request proposal)
                           (invalid-cold-crushing-strength-violations request proposal)))
