(ns refractorymfg.operation
  "RefractoryOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (RefractoryAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Refractory
  ...
  (:require [langgraph.graph :as g]
            ...
            [refractorymfg.advisor :as advisor]
...
(defn build
  "Compiles a RefractoryOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `refractorymfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RefractoryAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Refractory Plant Operations Governor -- independent censor.
      (g/add-node :govern
