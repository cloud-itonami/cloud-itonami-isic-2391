(ns refractorymfg.store
  "SSoT for the refractory products (fire brick, kiln lining, furnace
  lining materials) plant-operations coordination actor, behind a
  ...
  (`cloud-itonami-isic-2392`'s own `claymfg.store`, itself modeled on
  `cloud-itonami-isic-2431`'s `foundrymfg.store`), this build ships a
  ...
  refractory-products capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             kiln-fired lot of refractory brick or
                             lining material)'s product-type/weight/
                             thermal-shock-cycles/cold-crushing-
                             strength-mpa record. `:verified?` marks
    ...
    - `equipment`         -- a pressing-line/kiln-line unit's own
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled pressing-line/kiln-line-
                             equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`refractorymfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound refractory-product
                             shipment DRAFT (`refractorymfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (refractorymfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (refractorymfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :fireclay-brick
                :material "Standard Fireclay Brick"
                :weight-kg 40000.0 :thermal-shock-cycles 25.0 :cold-crushing-strength-mpa 45.0
                :verified? true :registered? true
                :shipped-weight-kg 8000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :high-alumina-brick
                :material "70% High-Alumina Brick"
                :weight-kg 6000.0 :thermal-shock-cycles 40.0 :cold-crushing-strength-mpa 80.0
                :verified? true :registered? true
                :shipped-weight-kg 5600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :magnesia-brick
                :material "Magnesia-Chrome Brick"
                :weight-kg 50000.0 :thermal-shock-cycles 12.0 :cold-crushing-strength-mpa 60.0
...
(defn- sample-equipment []
  {"kiln-001" {:id "kiln-001" :kind :tunnel-kiln
   ...
   "press-002" {:id "press-002" :kind :isostatic-press
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `refractorymfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `refractorymfg.registry` and
...
(defn sample-data!
  ...
  isostatic-press unit (blocks any maintenance scheduling against
