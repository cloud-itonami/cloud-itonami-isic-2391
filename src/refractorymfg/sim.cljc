(ns refractorymfg.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean refractory-
  products plant through intake -> maintenance scheduling (escalate/
  ...
  that tries to ACTUATE the pressing-line press or kiln line directly
  ...
  thermal-shock-cycles reading, and a production-batch patch with an
  implausible cold-crushing-strength reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-2 on press-002 (UNVERIFIED/unregistered isostatic press -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on kiln-001 with :actuate-kiln-pressing-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible thermal-shock-cycles reading -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:thermal-shock-cycles 999.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible cold-crushing-strength reading -> HARD hold ==")
    (println (exec-op actor "t14"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:cold-crushing-strength-mpa 999.0}}
                       coordinator))
