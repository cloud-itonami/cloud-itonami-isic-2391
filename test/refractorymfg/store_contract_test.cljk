(ns refractorymfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `refractorymfg.store` ns docstring for why a second
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (false? (:verified? (store/equipment-unit s "press-002"))))
    (is (false? (:registered? (store/equipment-unit s "press-002"))))
    (is (= ["kiln-001" "press-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :high-alumina-brick}})
    (is (= :high-alumina-brick (:product-type (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "press-002" :severity :high}})
