(ns refractorymfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the pressing-line press or kiln
  line directly... does NOT authorize or execute maintenance/kiln-line
  operations') implemented faithfully. The single invariant under
  test:

    RefractoryAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Refractory Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :high-alumina-brick}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :high-alumina-brick (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest kiln-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-kiln-pressing-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "kiln-001" :maintenance-type :kiln-lining-inspection
                                          :scheduled-date "2026-08-01" :actuate-kiln-pressing-line? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-thermal-shock-cycles-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:thermal-shock-cycles 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-thermal-shock-cycles} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:thermal-shock-cycles (store/batch db "batch-001"))) "fabricated thermal-shock-cycles never lands in the SSoT")))

(deftest invalid-cold-crushing-strength-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t12" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:cold-crushing-strength-mpa 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-cold-crushing-strength} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:cold-crushing-strength-mpa (store/batch db "batch-001"))) "fabricated cold-crushing-strength never lands in the SSoT")))
