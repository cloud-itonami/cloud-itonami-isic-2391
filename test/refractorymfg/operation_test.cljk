(ns refractorymfg.operation-test
  "Smoke tests for the compiled RefractoryOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `refractorymfg.governor-contract-test`; the Store contract in
  `refractorymfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "RefractoryOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t1"
                          {:op :log-production-batch :effect :propose :subject "batch-001"
                           :patch {:product-type :high-alumina-brick}}
                          coordinator)
